			...
			add: function(sendAll, request){
				...
				angular.forEach($scope.windows.data, function(window){
					angular.forEach(window.tabs, function(tab){
						...
						// Necessities
						var month, currentDayIndex, dayIndex, monthIndex, hour, date, rightNow, startingDate, futureDate = null;
						...
						if('laterToday' === request){
							...
							// If wakeUpThisTab is 'Every month'
								...
								if(rightNow.diff(startingDate) > 0){ startingDate.add(1, 'months'); }
							...
							// If wakeUpThisTab is 'Every year'
								...
								if(rightNow.diff(startingDate) > 0){ startingDate.add(1, 'years'); }
						...
						else if(tab.tmSelected){ $scope.tabs.snooze.current.data.push(snoozedTab); }
